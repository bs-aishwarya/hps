...
import 'package:hps/common/custom_toast_message.dart';
import 'package:hps/common/getPlacemart.dart';
import 'package:hps/common/time_format.dart';
import 'package:hps/data/repositories/auth_repositories.dart';
import 'package:hps/data/repositories/firebase_queries_for_markers/firebase_queries_for_markers.dart';
import 'package:hps/data/repositories/firebase_queries_for_regionMap/firebase_queries_for_regionMap.dart';
...
import 'package:hps/features/core/screens/MarkerMap/widgets/custom_marker.dart';
...
import 'package:hps/data/repositories/firebase_query_repository/firebase_query_for_users.dart';
import 'package:hps/features/core/screens/MarkerMap/AddMarkerDetailsScreen.dart';
...
class MarkerMapController extends GetxController {
  ...
  Future<void> initData() async {
    ...
    await settingsController.instance.getProfileImageFromBackend().then((
      value,
    ) {
  ...
  Future<void> getUserLocation() async {
    try {
      ...
      serviceEnabled =
          await MarkerMapController.instance.locationController
              .serviceEnabled();
  ...
  Future<void> createAndAddCurrMarker() async {
    try {
      ...
      BitmapDescriptor icon = await widgetToIcon(
      ...
      HalfMapController.instance.allHalfMapMarkers.add(
        currPosMarker,
      ); // Adding the Custom Marker in Home Screen
  ...
  Future<void> moveToCurrLocation(File? imageFile) async {
    ...
    try {
      await googleMapController.animateCamera(
        CameraUpdate.newLatLngZoom(currPos.value!, 16),
      );
    ...
    final marker = Marker(
      markerId: const MarkerId("currentLocation"),
      position: currPos.value!,
      icon: await CircularWidget(imageFile: imageFile).toBitmapDescriptor(
        logicalSize: const Size(50, 50),
    ...
    Marker marker = Marker(
      ...
      onTap: () async {
        ...
        customInfoWindowController.addInfoWindow!(
          infoWindow(desc, null, id, time, hasDelete),
          position,
        );

        Directory tempDir = await getTemporaryDirectory();
        String appDirPath = '${tempDir.path}/hps/MarkerImages/';
        ...
        customInfoWindowController.addInfoWindow!(
          infoWindow(desc, image, id, time, hasDelete),
          position,
        );
      },
      icon:
        ...
        child: Column(
          ...
          children: [
            ...
            hasDelete
                ? TextButton(
                  onPressed: () async {
                    ...
                    await FirebaseQueryForUsers()
                        ...
                        .onError(
                          (error, stackTrace) => showCustomToast(
                            context,
                    ...
                    await HalfMapController.instance.removeMarkerFromHalfMap(
                      "$markerid",
                    );
                ...
                : TextButton(
                  onPressed: () {},
                  child: const Text("Other's Marker"),
                ),
          ],
        ),
      ),
