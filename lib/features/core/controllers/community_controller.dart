...
import 'package:hps/utils/formatters/formatter.dart';
...
class CommunityController extends GetxController {
  ...
  Future<void> uploadImageToCommunityImages(
    ...
    try {
      final userFireStoreReferenceHD = storage.ref().child(
        "CommunityImages/HD/U$imageName",
      );

      final userFireStoreReferenceCompressed = storage.ref().child(
        "CommunityImages/Compressed/U$imageName",
      );

      final userFireStoreReferenceProfile = storage.ref().child(
        "CommunityImages/Profile/U$imageName",
      );

      final UploadTask uploadTask1 = userFireStoreReferenceHD.putFile(image);
      final UploadTask uploadTask2 = userFireStoreReferenceCompressed.putFile(
        compressedImage,
      );
      final UploadTask uploadTask3 = userFireStoreReferenceProfile.putFile(
        profile,
      );

      await uploadTask1.whenComplete(
        () => print('HD Image uploaded successfully'),
      );
  ...
  Future<List<File?>> getImageFromCommunity(
    ...
    try {
      ...
      final String imagePath = '$tempPath/hpstempImages';
      ...
      Reference storageReference1 = FirebaseStorage.instance.ref().child(
        "CommunityImages/Compressed/$postId",
      );
      Reference storageReference2 = FirebaseStorage.instance.ref().child(
        "CommunityImages/Profile/$postId",
      );
      ...
      CommunityController
              .instance
              .listOfCommunityPost
              .value[listIndex]["Generic_Post_Info"]["image"] =
          imageFile1;
      CommunityController
              .instance
              .listOfCommunityPost
              .value[listIndex]["Generic_Post_Info"]["prof_image"] =
          imageFile2;
  ...
  Future<void> getPostFromCommunity() async {
    ...
    try {
      final QuerySnapshot<Map<String, dynamic>> querySnapshot =
          await storageRef
              .orderBy(
                "Generic_Post_Info.last_interaction_time",
                descending: true,
              )
              .limit(loadMorelimit)
              .get();

      var listOfCommunityPostRetrieved =
  ...
  Future<String> reportPost(
    ...
    try {
      var url = Uri.https(
        'hpsbackend-vu6qon67ia-el.a.run.app',
