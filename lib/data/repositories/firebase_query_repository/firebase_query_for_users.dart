...
class FirebaseQueryForUsers {
  ...
  Future<String> uploadImageToMarkerImages(File image, int imageName) async {
    try {
      final userFireStoreReference = storage.ref().child(
        "MarkerImages/$imageName",
      );
  ...
  Future<void> addMarkerToUser(
    ...
    try {
      ...
      await FirebaseQueryForMarkers().addMarkerToMarkers(
        json,
        userId,
        randomMarkerID,
      );
  ...
  Future<void> deleteMarkerFromFirestoreUsers(int markerId) async {
    try {
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        await FirebaseQueryForMarkers().deleteMarkerFromFirestoreMarkers(
          markerId,
        );
